...
pub(crate) fn read_database(
    ...
    // Prefer a normal read-only connection so a live WAL is honoured. On
    // filesystems where SQLite shared memory is unavailable (for example Docker
    // Desktop bind mounts on Windows/macOS) that fails, so fall back to opening
    // the database immutably, which reads the last checkpointed state.
    if let Ok(connection) = open_read_only(path) {
        if let Ok(usage) = read_usage(&connection, cutoff_ms, pricing) {
            return Ok(DatabaseRead::Usable(usage));
        }
    }
    // The immutable read can race a live writer, so retry a few times.
    let mut last_error = Err(());
    for attempt in 0..3 {
        match open_immutable(path).and_then(|connection| {
            read_usage(&connection, cutoff_ms, pricing).map(DatabaseRead::Usable)
        }) {
...
fn read_usage(
    ...
    Ok(DatabaseUsage { records })
}

pub(crate) fn has_hosted_usage(path: &Path) -> Result<bool, ()> {
    if let Ok(connection) = open_read_only(path) {
        if let Ok(found) = query_hosted_usage(&connection) {
            return Ok(found);
        }
    }
    let connection = open_immutable(path)?;
    query_hosted_usage(&connection)
}

fn query_hosted_usage(connection: &Connection) -> Result<bool, ()> {
    let schema = inspect_schema(connection)?;
...
/// Open a database without touching the WAL/SHM side files. Used as a fallback
/// on filesystems that cannot provide SQLite shared memory (Docker Desktop bind
/// mounts). It reads the last checkpointed state, so the very latest local
/// usage may be missing until the writer checkpoints.
