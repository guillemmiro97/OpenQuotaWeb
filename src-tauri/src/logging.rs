...
static CONSOLE_OUTPUT: AtomicBool = AtomicBool::new(false);
...
/// Mirror log lines to stdout (used by the headless web server so logs are
/// available through `docker logs`).
pub fn enable_console_output() {
    CONSOLE_OUTPUT.store(true, Ordering::Relaxed);
}
...
pub fn emit(level: LogLevel, tag: &str, arguments: fmt::Arguments<'_>) {
    ...
    if cfg!(debug_assertions) || CONSOLE_OUTPUT.load(Ordering::Relaxed) {
        println!("{line}");
    }
